...
parser.add_argument('--searchwindow', action='store',type=int, default=5*60,help='Seconds it should search back in the log')
	...
	for line in reverse_lines(logfile):
		...
		if result:
			...
			if timestamp <= stoptime:
				return retval
		for regex in regex_good:
			if regex.search(line):
				return 0
		for regex in regex_hearthbeat:
			if regex.search(line):
				retval = 0
		for regex in regex_bad:
			if regex.search(line):
				return 1
	return retval
